...
class MayaSessionCollector(HookBaseClass):
    ...
    def _collect_cameras(self, parent_item):
        ...
        scene_cames = []
        # If cameras were added in a group. this add the group into taken account
        all_nodes = cmds.ls(assemblies=True)
        for nodes in cmds.listRelatives(all_nodes, ad=True, fullPath=True):
            if cmds.ls(nodes, type='camera'):
                st_cam = [cmds.camera(cmds.listRelatives(nodes, parent=True)[0], startupCamera=True, q=True)]
                if st_cam[0] is False:
                    stop = True
                    while stop:
                        get_root_node = cmds.listRelatives(nodes, p=True, fullPath=True)
                        if get_root_node is not None:
                            nodes = get_root_node[0]
                        else:
                            stop = False
                    cam_node = ( nodes.split('|')[-1] if nodes.startswith('|') else nodes )
                    scene_cames.append(cam_node)
                
        for camera_shape in scene_cames:
